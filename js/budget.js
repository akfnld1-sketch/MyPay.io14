  ...
  try {
    ...
    for (let day = 1; day <= dim; day++) {
      ...
      try {
        ...
        // 알바: 시급 × 시간
        (data.alba || []).forEach(it => { total += Math.round((it.wage||0) * (it.hours||0)); });
        // 배달/대리: 건수 × 단가
...
function calcZeroBalanceDate() {
  ...
  // 절약 시뮬레이션: 30% 절약 시 추가 생존 일수
  const savedDaily  = dailyAvg * 0.3;
  const extraDays   = savedDaily > 0 ? Math.floor(remaining / (dailyAvg - savedDaily)) - daysLeft : 0;
  const savingAmt30 = Math.round(savedDaily * daysLeft); // 절약 가능 금액
  ...
  const savingAmt30 = dailyAvg > 0 ? Math.round(dailyAvg * 0.3 * daysLeft) : 0;
  return { date: label, daysLeft, dailyAvg, usable, income, fixed, variable, remaining, extraDays, savingAmt30 };
}
...
function renderBudgetPage() {
  ...
  for(let d=1;d<=dim;d++){
    ...
    try{
      ...
      if(!raw) continue;
      const data=JSON.parse(raw);
      (data.alba||[]).forEach(it=>{ njobAlbaTotal+=(it.amount||Math.round((it.wage||0)*(it.hours||0))); });
  ...
  <div style="background:${wBgs[wLevel]};border:2px solid ${wBorders[wLevel]};border-radius:12px;
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
      ...
      <div style="flex:1;">
        ...
        <div style="font-size:12px;color:var(--text2);margin-top:2px;">
          ${calc.daysLeft>0?`잔고 소진 예상 <b style="color:${wColors[wLevel]};">${calc.date}</b> (${calc.daysLeft}일 후)`:`<b style="color:var(--red);">${calc.date}</b>`}
        </div>
      </div>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      <div style="flex:1;min-width:120px;background:var(--surface);border-radius:8px;padding:8px 10px;">
        ...
        <div style="font-size:14px;font-weight:700;font-family:'JetBrains Mono';">
          ${calc.dailyAvg>0?Math.round(calc.dailyAvg).toLocaleString()+'원':'기록 없음'}
        </div>
      </div>
      <div style="flex:1;min-width:120px;background:var(--surface);border-radius:8px;padding:8px 10px;">
        <div style="font-size:10px;color:var(--text3);">남은 가용 잔고</div>
        <div style="font-size:14px;font-weight:700;font-family:'JetBrains Mono';color:${calc.remaining>0?'var(--green)':'var(--red)'};">
          ${calc.remaining>0?calc.remaining.toLocaleString()+'원':'초과!'}
        </div>
      </div>
    </div>
    ${wLevel>=1?`
    <div style="margin-top:8px;padding:6px 10px;border-radius:6px;
                background:rgba(0,0,0,.05);font-size:11px;color:var(--text2);">
      ${['','💡 소비 속도를 조절하세요','🛑 비필수 지출을 줄이세요','⛔ 추가 지출 금지 상태입니다'][wLevel]}
    </div>`:''}
  </div>

  <!-- 절약 시뮬레이션 카드 -->
  ${calc.dailyAvg>0&&calc.daysLeft>0?`
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;
              padding:14px 16px;margin-bottom:12px;">
    <div style="font-size:13px;font-weight:700;margin-bottom:10px;">🎯 절약 시뮬레이션</div>
    <div style="font-size:12px;color:var(--text2);margin-bottom:8px;">
      현재 소비 패턴에서 <b>30% 절약</b> 시:
    </div>
    <div style="display:flex;gap:8px;">
      <div style="flex:1;background:rgba(61,214,140,.08);border:1px solid rgba(61,214,140,.2);
                  border-radius:8px;padding:10px;text-align:center;">
        <div style="font-size:10px;color:var(--text3);margin-bottom:4px;">생존 기간 연장</div>
        <div style="font-size:20px;font-weight:900;color:var(--green);">+${calc.extraDays}일</div>
      </div>
      <div style="flex:1;background:rgba(79,124,255,.08);border:1px solid rgba(79,124,255,.2);
                  border-radius:8px;padding:10px;text-align:center;">
        <div style="font-size:10px;color:var(--text3);margin-bottom:4px;">절약 가능 금액</div>
        <div style="font-size:16px;font-weight:900;color:var(--accent);">${(calc.savingAmt30||0).toLocaleString()}원</div>
      </div>
    </div>
    <div style="margin-top:8px;padding:6px 10px;background:rgba(255,209,102,.08);border-radius:6px;
                font-size:11px;color:var(--text3);">
      💡 하루 ${Math.round(calc.dailyAvg*0.3).toLocaleString()}원만 아껴도 잔고가 ${calc.extraDays}일 더 유지돼요
    </div>
  </div>`:''}
  ...
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;
    ...
    <div style="display:flex;flex-direction:column;gap:5px;">
      ...
      <div style="display:flex;justify-content:space-between;padding:5px 8px;
        ...
        <span style="font-size:12px;color:var(--text2);">⏰ 알바</span>
        <span style="font-size:12px;font-weight:700;color:var(--orange);">${njobAlbaTotal>0?'+'+njobAlbaTotal.toLocaleString()+'원':'0원'}</span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:5px 8px;
