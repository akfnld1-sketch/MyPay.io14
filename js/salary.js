...
function getPayData(){
  ...
  return {basePay, normalH, twd, wDays, lDays, halfDays, absDays,
    ...
    netPay, grossPay, ins, tax, finalPay, isPerfect, perfAmt,
...
function renderIncomePage(){
  ...
  function makeNjobSection(label, icon, color, items, gross){
    const net = Math.round(gross * 0.967);
    ...
    const inner = items.length>0 ? rows + `
      <div style="display:flex;justify-content:space-between;padding:8px 0;border-top:1px solid var(--border);margin-top:4px;">
        <span style="font-size:12px;color:var(--text2);">${label.includes('알바')?'고용보험 공제 후':'3.3% 공제 후'}</span>
        <span style="font-size:14px;font-weight:700;color:var(--green);">${label.includes('알바')?Math.round(gross*0.991).toLocaleString():Math.round(gross*0.967).toLocaleString()}원</span>
      </div>` :
